...
<div class="back-cover" x-show="showInsuranceDetailsModal" x-transition.opacity x-cloak>
    <div class="w-90 h-fit max-h-[calc(100vh-100px)] overflow-auto flex flex-col bg-white p-5 gap-5 rounded-2xl z-20 font-[shabnam]"
        x-data="{ hideModal() { showInsuranceDetailsModal = false;
        ...
        <div class="w-full flex items-center gap-2 text-gray-500">
            ...
            تاریخ افزودن:
            <p class="text-black">
                {{ jalali($insurance->createdAt ?? '-', 'H:i - Y/m/d') }}
            </p>
        </div>
