<div class="back-cover" x-show="showDeleteShippingMessage" x-transition.opacity x-cloak>
    <div class="w-80 h-fit flex flex-col bg-white p-5 gap-5 rounded-2xl z-20"
        x-data="{ hideModal() { showDeleteShippingMessage = false; selectedShippingForDelete = null } }"
        ...
        <p class="w-full h-auto m-0 p-0 font-[shabnam] text-[17px]">
            آیا از حذف پست ارسال اطمینان دارید؟
        </p>
        <div class="flex flex-nowrap gap-[10px] justify-end items-center w-full">
            ...
            <x-blade.manager.text-button value="بله" target="delete" wire:click="delete(selectedShippingForDelete)" />
        </div>
    </div>
</div>