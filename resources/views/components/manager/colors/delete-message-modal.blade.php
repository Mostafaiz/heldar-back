...
<div class="back-cover" x-show="showDeleteColorModal" x-transition.opacity x-cloak>
    <div class="w-80 h-fit flex flex-col bg-white p-5 gap-5 rounded-2xl z-20"
        x-data="{ hideModal() { showDeleteColorModal = false; selectedColorForDelete = null } }"
        ...
        <div class="flex flex-nowrap gap-[10px] justify-end items-center w-full">
            ...
            <x-blade.manager.text-button value="بله" target="delete"
                x-on:click="$wire.delete(selectedColorForDelete)" />
        </div>
    </div>
</div>