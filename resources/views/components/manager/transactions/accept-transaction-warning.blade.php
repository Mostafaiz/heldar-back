<div class="back-cover fixed!" x-show="showAcceptTransactionWarning" x-transition.opacity x-cloak>
    <div class="w-80 h-fit flex flex-col bg-white p-5 gap-5 rounded-2xl z-20" x-data="{
        hideModal() {
            showAcceptTransactionWarning = false;
            selectedTransactionForAorR = null
        }
    }"
        x-on:click.outside="hideModal()" x-on:success.window="hideModal()">
        <h2 class="w-full h-auto m-0 p-0 font-[500] font-[shabnam] text-green-600 text-[22px]">
        ...
        <div class="flex flex-nowrap gap-[10px] justify-end items-center w-full">
            ...
            <x-blade.manager.filled-button value="بله" target="acceptTransaction"
                x-on:click="$wire.acceptTransaction(selectedTransactionForAorR)" />
        </div>
    </div>
</div>
