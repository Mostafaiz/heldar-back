<div class="w-full h-50 flex flex-nowrap gap-4 lg:gap-6" x-data="{ loading: false }"
    x-on:stop-loading.window="loading = false">
    <div class="w-40 h-full flex flex-col gap-4 justify-between">
    ...
    <div class="w-full flex flex-col justify-between gap-4">
        <div class="w-full flex flex-col gap-3">
            <p
                class="w-full font-shabnam font-[500] text-sm lg:text-base line-clamp-3 overflow-hidden text-neutral-dark">
                {{ $product->name }}
            </p>
            @if (isset($product->pattern->name))
                <div class="w-full flex flex-nowrap items-center gap-2">
                    ...
                    <span class="font-shabnam text-xs text-gray-600">{{ $product->pattern->name }}</span>
                </div>
            @endif
